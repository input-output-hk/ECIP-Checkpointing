...
import Blockchain.Common.Parsers
import Blockchain.Common.TopHandler
import Blockchain.Config.Logging (createLoggingFeature)
import Blockchain.Config.Types
import Blockchain.Node.Features.Node
...
initializeAllFeatures nCli@NodeCLI { configFp = ncFp } = do
  ...
  pure ( [loggingFeature, nodeFeature] :: [CardanoFeature],
         nodeLayer)
