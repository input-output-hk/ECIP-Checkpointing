...
module Blockchain.Common.Conversions (
  integerFromHex,
  integerToHex,
  bytesToHex,
  bytesFromHex,
...
integerToBytes len n =
  if (BS.length padded > len)
    then error $ "Integer " <> show n <> " to large to fit in " <> show len <> " bytes"
    else B.Bytes padded
  where
...
normalizeHex s =
  ...
  where
    normalized = if T.isPrefixOf "0x" s then T.drop 2 s else s
    validChars = Set.fromList $ ['0'..'9'] <> ['a'..'f'] <> ['A'..'F']
    inputAsSet = Set.fromList $ T.unpack normalized
    validHex = Set.difference inputAsSet validChars == Set.empty
