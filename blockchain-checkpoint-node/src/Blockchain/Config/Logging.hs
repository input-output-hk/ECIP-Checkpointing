...
module Blockchain.Config.Logging
  ( LoggingLayer (..)
  , LoggingFlag (..)
...
loggingCLIConfiguration mfp captureMetrics' =
    case mfp of
      Nothing ->
        fmap (LoggingDisabled,) $ LoggingConfiguration <$> basicConfig <*> pure True
      Just fp ->
...
loggingCardanoFeatureInit disabled' conf = do
  ...
  (baseTrace, switchBoard) <- setupTrace_ logConfig "blockchain-checkpoint"
