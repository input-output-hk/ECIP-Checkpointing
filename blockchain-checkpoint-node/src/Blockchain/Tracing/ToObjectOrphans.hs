...
module Blockchain.Tracing.ToObjectOrphans
  ( WithTip (..)
  , showTip
  , showWithTip
  ) where
...
instance DefinePrivacyAnnotation (MorphoStateTrace c ext)
instance DefineSeverity (MorphoStateTrace c ext) where
  defineSeverity (MorphoStateTrace (MorphoLedgerState _)) =
    Debug

instance DefinePrivacyAnnotation (TimeTravelErrorTrace c ext)
...
instance (MorphoCrypto c, Typeable ext) => Transformable Text IO (MorphoStateTrace c ext) where
  trTransformer StructuredLogging verb tr = trStructured verb tr
  trTransformer TextualRepresentation _verb tr = Tracer $ \s ->
    traceWith tr =<< LogObject <$> pure mempty
                               <*> mkLOMeta (defineSeverity s) (definePrivacyAnnotation s)
...
instance Transformable Text IO (TimeTravelErrorTrace c ext) where
  trTransformer StructuredLogging verb tr = trStructured verb tr
  trTransformer TextualRepresentation _verb tr = Tracer $ \s ->
    traceWith tr =<< LogObject <$> pure mempty
                               <*> mkLOMeta (defineSeverity s) (definePrivacyAnnotation s)
...
instance (MorphoCrypto c, Typeable ext) => ToObject (MorphoStateTrace c ext) where
  toObject _verb (MorphoStateTrace (MorphoLedgerState (st @ (MorphoState _ _ _ tip _)))) =
    mkObject [ "kind" .= String "MorphoStateUpdate"
             , "state" .= show st
             , "tip" .= showTip NormalVerbosity tip ]

instance ToObject (TimeTravelErrorTrace c ext) where
  toObject _verb (TimeTravelErrorTrace err) =
