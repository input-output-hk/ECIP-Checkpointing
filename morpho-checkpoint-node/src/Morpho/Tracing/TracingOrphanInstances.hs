...
instance (HashAlgorithm h, BftCrypto c) => HasTextFormatter (ExtractStateTrace h c) where
  ...
  formatText (ExtractTxErrorTrace err) _ = pack $
    "Error while trying to extract Tx from PoW BlockRef: " ++ show err
  formatText (WontPushCheckpointTrace reason) _ = pack $
...
instance ( Show (GenTx blk), Show (GenTxId blk), ToObject (ApplyTxErr blk), Show (ApplyTxErr blk), ToObject (GenTx blk),
...
instance ( tx ~ GenTx blk
  ...
  formatText = \case
    ...
    TraceForgedBlock slotNo _ blk _ -> const $
