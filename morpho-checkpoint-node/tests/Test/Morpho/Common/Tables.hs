...
-- code vendored from: https://github.com/marcin-rzeznicki/hspec-tables/blob/45c07370a08b7079a74c9145dcbed2dc2d581efa/src/Test/Hspec/Tables.hs
-- at the time of writing it requires a newer GHC version than we use
...
-- >    (shouldBe . reverse)
module Test.Morpho.Common.Tables
  ( Table (..),
    byExample,
    testTable,
    byExample1Col,
...
instance Table (SingleColumn a) where
  type Header (SingleColumn a) = String
  type Forall (SingleColumn a) p = a -> p
  apply f (SingleColumn a) = f a
...
byExample1Col ::
  ...
  String ->
  [r] ->
  Forall (SingleColumn r) a ->
  SpecWith (Arg a)
byExample1Col h r = byExample h (SingleColumn <$> r)
