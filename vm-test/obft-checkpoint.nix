    ...
    miner = { ... }: {
      ...
      systemd.services.mantis-node = {
        ...
        script = let
          ...
          privateKey = pkgs.runCommandNoCC "mantis-private-key" {
            nativeBuildInputs = [ mantis ];
          } ''
            mantis cli generate-private-key | tee /dev/stderr | tail -1 > $out
          '';

          address = pkgs.runCommandNoCC "mantis-address" {
            nativeBuildInputs = [ mantis ];
          } ''
            mantis cli derive-address "$(cat ${privateKey})" | tee /dev/stderr | tail -1 > $out
          '';

          genesis = pkgs.runCommandNoCC "genesis.json" {
            nativeBuildInputs = [ mantis ];
          } ''
            mantis cli generate-allocs --balance=42 --address="$(cat ${address})" | tee /dev/stderr | tail -1 > $out
          '';
          ...
          '';
        in ''
          privateKey=$(mantis cli generate-private-key | tail -1)
          address=$(mantis cli derive-address "$privateKey" | tail -1)
